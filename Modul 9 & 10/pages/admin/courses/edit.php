...
$course;
if (isset($_GET["id"])) {
  $course = show($connect, $_GET["id"]);
}

if (isset($_POST["updateCourse"])) {
  update($connect, $_GET["id"]);
}
?>
      ...
      <div class="bg-white p-8 rounded shadow">
        <?php foreach ($course as $row) : ?>
          <form action="" method="POST">
            <div class="mb-5">
              <img src="./../../../images/courses/<?= $row["images"] ?>" alt="courseImages" class="object-cover h-[200px] w-[300px] mx-auto rounded border" style="height: 200px; width: 300px;">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-5">
              ...
              <input type="text" class="border rounded w-full py-2 px-3 bg-gray-50" id="name" name="name" value="<?= $row["name"] ?>">
            </div>
            <div class="mb-7">
              <label for="description" class="font-semibold">Description : </label>
              <textarea name="description" id="description" class="w-full rounded border h-[100px] px-3 py-2 bg-gray-50"><?= $row["description"] ?></textarea>
            </div>
            <div class="text-right">
              <button name="updateCourse" class="bg-purple-500 hover:bg-purple-400 px-5 py-2 rounded text-white text-sm">
                Update
              </button>
            </div>
          </form>
